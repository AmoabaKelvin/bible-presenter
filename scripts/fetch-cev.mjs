// Standalone Node ESM script that downloads the full Contemporary English
// Version (CEV) from the bolls.life Bible API (translation "CEVD" — CEV 2006)
// and writes it to public/bibles/cev.json so the app can ship it as a baked-in
// offline translation. The eightlabs API does not serve the CEV, so this
// alternate source is used. One request per chapter (1189 total), throttled to
// ~6 concurrent. DO NOT run casually.
//
...
//
// CEV © 1991, 1992, 1995 by American Bible Society. Bundled here for personal,
// non-commercial use only.
//
// Usage: node scripts/fetch-cev.mjs
...
async function run() {
  ...
  const outPath = join(outDir, "cev.json")
  await mkdir(outDir, { recursive: true })
  await writeFile(outPath, JSON.stringify({ version: "CEV", chapters }))
