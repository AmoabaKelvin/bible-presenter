// Standalone Node ESM script that downloads the full Berean Standard Bible
// (apiId "englishberean") from the eightlabs API and writes it to
// public/bibles/bsb.json so the app can ship a baked-in modern offline
// translation. BSB is public domain, so it is safe to redistribute/bundle.
// Mirrors fetch-kjv.mjs. Throttled to ~6 concurrent chapter fetches.
// DO NOT run casually: it makes ~1189 network requests.
//
// Usage: node scripts/fetch-bsb.mjs
...
const BSB_API_ID = "englishberean"
const CONCURRENCY = 6
...
async function fetchChapter(task) {
  ...
  const url = `${BIBLE_API_BASE}/verses/${apiId}.${chapter}.1-${verseCount}?translation=${BSB_API_ID}`
...
async function run() {
  ...
  const outPath = join(outDir, "bsb.json")
  await mkdir(outDir, { recursive: true })
  await writeFile(outPath, JSON.stringify({ version: "BSB", chapters }))
