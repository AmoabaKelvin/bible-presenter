...
The main process exposes a narrow preload bridge. Renderers keep sandboxing and context isolation enabled, and have no Node access. File access permits reading user-selected files and folders through their handles, including across the two windows and after a restart; write access is denied. Electron re-checks a handle's grant with no window attached, so that decision rests on the app's origin alone (`fileReadAllowed` in `desktop/security.cjs`). Microphone access uses the macOS permission prompt. The helper only receives audio clips and does not own microphone capture.
...
The installed app updates itself. Shortly after launch and every six hours it reads this repository's GitHub releases, and when a newer `desktop-v*` release carries a `FlowCast-<version>-arm64.dmg`, it downloads it, checks it against the SHA-256 GitHub publishes for that asset, confirms the code signature, bundle identifier, and version, and keeps it under `~/Library/Application Support/FlowCast/update`. The new version replaces the app when FlowCast quits; nothing restarts or prompts on its own, because a service may be live. FlowCast > Check for Updates… does the same on demand and offers to restart. To ship an update: raise `version` in `package.json`, then push the matching `desktop-v<version>` tag. Saved shows, media, and accounts live outside the app and carry over. Electron's built-in updater is not used because it refuses ad-hoc signatures; until builds are signed with a Developer ID, macOS treats each update as a new app and asks once more for Keychain ("FlowCast Safe Storage") and microphone access. Copies run from the disk image, or from a folder the user cannot write to, do not update.
...
node desktop/update-check.mjs dist/FlowCast-<version>-arm64.dmg
```

The smoke test launches the staged application with an isolated profile under the macOS temporary directory. It exercises the private server, callback rejection, renderer isolation, operator/output storage synchronization, reading a video and a folder through file handles, native fullscreen, the real Swift WebSocket listener and token rejection, and persistence across a page reload. It captures operator/output screenshots. It does not request microphone access or download models. `node desktop/update-check.mjs` updates a throwaway stand-in for the installed app from a local release feed serving the packaged DMG: it must refuse a wrong checksum, then leave an intact, correctly signed new version in place. `node desktop/voice-check.mjs` transcribes synthesized speech through the staged helper using the app's real vocabulary; it downloads models on first use, or into a scratch directory given with `--models`.
